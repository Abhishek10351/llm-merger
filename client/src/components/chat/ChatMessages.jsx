            ...
            <div className="flex flex-col space-y-4">
                {messages.map((message, index) => (
                    <div key={index} className="flex flex-col space-y-2">
                        <div className="px-4 py-2 rounded-lg self-end bg-primary text-primary-foreground max-w-[80%] md:max-w-[60%]">
                            <p className="text-sm">{message["user_content"]}</p>
                        </div>
                        {message[selectedLlm] && (
                            <div className="px-4 py-3 rounded-lg self-start bg-background text-foreground border border-border shadow-sm max-w-[90%] md:max-w-[80%]">
